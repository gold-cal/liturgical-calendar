...
import com.liturgical.calendar.activities.EventTypePickerActivity
...
fun Context.updateWidgets() {
    val widgetIDs = AppWidgetManager.getInstance(applicationContext)?.getAppWidgetIds(ComponentName(applicationContext,
        MyWidgetMonthlyProvider::class.java)) ?: return
...
fun Context.updateDateWidget() {
    val widgetIDs = AppWidgetManager.getInstance(applicationContext)?.getAppWidgetIds(ComponentName(applicationContext,
        MyWidgetDateProvider::class.java)) ?: return
    if (widgetIDs.isNotEmpty()) {
...
fun Context.scheduleListWidgetRefresh(activate: Boolean) {
    ...
    if (activate) {
        if (!config.widgetUpdate) config.widgetUpdate = true
        val updateCheckInterval = AlarmManager.INTERVAL_HALF_HOUR
        try {
            alarmManager.setInexactRepeating(AlarmManager.RTC,
                System.currentTimeMillis() + updateCheckInterval,
                updateCheckInterval, pendingIntent)
        } catch (ignored: Exception) {
            //toast("Failed to schedule widget refresh!")
        }
    } else {
        config.widgetUpdate = false
    }
}
...
fun Context.getEventListItems(events: List<Event>, addSectionDays: Boolean = true, addSectionMonths: Boolean = true): ArrayList<ListItem> {
    ...
    sorted.forEach {
        ...
        isSpecialEvent = it.source == SOURCE_CONTACT_BIRTHDAY || it.source == SOURCE_CONTACT_ANNIVERSARY
        if (addSectionMonths) {
        ...
        if (code != prevCode && addSectionDays) {
            ...
            val listSectionDay = ListSectionDay(day, code, config.widgetDayColor, isToday, !isToday && it.startTS < now)
...
/*fun Context.getTodayIndex() {
    val firstNonPastSectionIndex = config.displayPastEvents
}*/
